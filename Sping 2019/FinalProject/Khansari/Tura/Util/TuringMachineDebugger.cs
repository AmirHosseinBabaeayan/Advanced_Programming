...
namespace Tura.Util
{
    class TuringMachineDebugger
    {
        TuringMachine ContainingMachine;
        public TuringMachineDebugger(TuringMachine m)
        {
            ContainingMachine = m;
        }
        ...
        bool AreEdgesInterfering(Edge<TuringCondition> e1, Edge<TuringCondition> e2)
        {
            if (e1 != e2 && e1.Source == e2.Source)
            {
                foreach (TuringCondition c in e1.GetConditions)
                {
                    foreach (TuringCondition C in e2.GetConditions)
                    {
